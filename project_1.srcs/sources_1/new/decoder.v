...
`endif
    ...
    output halt, // 1 if this inst is a halt inst.
    output reg illegal
    );
...
`endif
            ...
            6'b010011: begin // or
                `decode(rs_wire, rt_wire, rd_wire, `REG_WB, `INST_OR, `ALU_OP_OR, `ZERO_EXT, `B_REG, `WB_ALU);
`ifdef DEBUG_DEC
                if (!stall)
...
`endif
            ...
            6'b111000: begin // j
                `decode(0, 0, 0, `REG_N, `INST_J, `ALU_OP_EQ, `ZERO_EXT, `B_REG, `WB_ALU);
`ifdef DEBUG_DEC
                if (!stall)
